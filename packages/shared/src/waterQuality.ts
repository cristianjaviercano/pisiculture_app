export type WaterQualityStatus = 'ok' | 'warn' | 'critical';
...
export const CTQ_PARAMS: Record<WaterParamKey, CTQParam> = {
  temperatura: {
  ...
  tan: {
    key: 'tan', label: 'TAN (Amoniaco Total)', unit: 'mg/L',
  ...
  nitrito: {
    key: 'nitrito', label: 'Nitrito (NO₂⁻)', unit: 'mg/L',
  ...
  alcalinidad: {
    key: 'alcalinidad', label: 'Alcalinidad', unit: 'mg/L CaCO₃',
  ...
  disco_secchi: {
    key: 'disco_secchi', label: 'Disco Secchi', unit: 'cm',
  ...
  salinidad: {
    key: 'salinidad', label: 'Salinidad', unit: 'ppt',
...
export interface WaterReadingEvaluation {
  systemStatus: WaterQualityStatus;
  hasBlocking: boolean;
  results: Partial<Record<WaterParamKey, CTQResult>>;
  alerts: Array<CTQResult & { key: WaterParamKey; label: string; value: number; unit: string }>;
}

export function evaluateCTQ(key: WaterParamKey, value: number): CTQResult {
  const p = CTQ_PARAMS[key];

  if (p.hard_min !== null && value < p.hard_min) {
    return {
      status: 'critical',
      message: p.hard_message ?? `${p.label} por debajo del límite crítico (${p.hard_min} ${p.unit})`,
      blocking: p.blocking,
    };
  }
  if (p.hard_max !== null && value > p.hard_max) {
    return {
      status: 'critical',
      message: p.hard_message ?? `${p.label} por encima del límite crítico (${p.hard_max} ${p.unit})`,
      blocking: p.blocking,
    };
  }
  if (p.warn_min !== null && value < p.warn_min) {
    return { status: 'warn', message: p.tip_low ?? `${p.label} en zona de advertencia`, blocking: false };
  }
  if (p.warn_max !== null && value > p.warn_max) {
    return { status: 'warn', message: p.tip_high ?? `${p.label} en zona de advertencia`, blocking: false };
  }
  if (p.optimal_min !== null && value < p.optimal_min) {
    return { status: 'warn', message: p.tip_low ?? `${p.label} levemente bajo`, blocking: false };
  }
  if (p.optimal_max !== null && value > p.optimal_max) {
...
export function evaluateWaterReading(reading: WaterReadingInput): WaterReadingEvaluation {
  const results: Partial<Record<WaterParamKey, CTQResult>> = {};
  let systemStatus: WaterQualityStatus = 'ok';
  let hasBlocking = false;
  const alerts: WaterReadingEvaluation['alerts'] = [];

  for (const key of Object.keys(CTQ_PARAMS) as WaterParamKey[]) {
    const value = reading[key];
    if (value === undefined || value === null) continue;

    const r = evaluateCTQ(key, value);
    results[key] = r;

    if (r.status === 'critical') {
      systemStatus = 'critical';
      if (r.blocking) hasBlocking = true;
    } else if (r.status === 'warn' && systemStatus !== 'critical') {
      systemStatus = 'warn';
    }

    if (r.status !== 'ok') {
      alerts.push({ ...r, key, label: CTQ_PARAMS[key].label, value, unit: CTQ_PARAMS[key].unit });
    }
  }
